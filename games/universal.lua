...
local function serverHop(pointer, filter)
	...
	if not pointer then
		notif('KitV4', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('KitV4', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('KitV4', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('KitV4', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end
...
local hash = loadstring(downloadFile('KitVape/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('KitVape/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('KitVape/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
	tagcallback = {},
...
run(function()
		...
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				vape.Uninject = function()
					notif('KitV4', 'No escaping the private members :)', 10)
				end
			end
		end
	end
	...
	function whitelist:announce(text)
		...
		if success then
			return sendToast({
				toastTitle = text,
				iconImage = getcustomasset('KitVape/assets/new/vape.png'),
				swipeUpDismiss = true,
		...
		icon.Image = getcustomasset('KitVape/assets/new/vape.png')
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('KitVape/profiles/whitelist.json') and readfile('KitVape/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('KitVape/profiles/whitelist.json', whitelist.textdata)
				end)
			end
		end
	end
...
run(function()
	...
	local Hooks = {
		...
		ScreenPointToRay = function(args)
			...
			if Projectile.Enabled and rand.NextNumber(rand, 0, 100) <= ProjectileChance.Value then
	...
	Projectile = SilentAim:CreateToggle({
		...
		Function = function(callback)
			...
			ProjectileChance.Object.Visible = callback
		end
	})
	...
	ProjectileChance = SilentAim:CreateSlider({
		Name = 'Projectile Chance',
		Min = 0,
		Max = 100,
		Default = 100,
		Darker = true,
		Visible = false,
		Suffix = '%',
		Tooltip = 'Probability that projectile prediction is applied per shot. 100% = always, 0% = never.'
	})
end)
...
run(function()
	...
	HighJump = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				-- Auto-enable AntiFlag when HighJump activates. HighJump
				-- modes 'CFrame' and 'Instant' teleport the root, and
				-- 'Velocity' / 'Impulse' spike Y velocity past server
				-- limits - both trip the LagController + the per-tick
				-- reconciliation flag. We just toggle AntiFlag on; if
				-- the user doesn't have premium access the AntiFlag
				-- module won't exist in vape.Modules and the pcall
				-- silently no-ops.
				pcall(function()
				...
				-- Mutual exclusion with Fly. If the user has Fly
				-- toggled on when they turn HighJump on, disable Fly
				-- first. Having both running at once causes conflicting
				-- RootPart velocity writes (Fly fights with HighJump's
				-- vertical spike), which the server flags immediately.
				-- pcall covers games where vape.Modules.Fly doesn't
				-- exist (most non-BedWars PlaceIds).
				pcall(function()
...
-- Invisible module moved to Auto TP in premium.lua
...
run(function()
	...
	local function getAttackData()
		...
		-- Fallback: if no TouchTransmitter, use the tool's Handle or first BasePart
			...
			if callback then
				repeat
					...
					if interest and tool then
						-- Always get a real BasePart for firetouchinterest
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								if not (v.Character and v.RootPart) then continue end
								local delta = (v.RootPart.Position - selfpos)
								local flatDelta = (delta * Vector3.new(1, 0, 1))
								-- Clamp dot to [-1,1] so math.acos never returns NaN
								local dot = math.clamp(localfacing:Dot(flatDelta.Unit), -1, 1)
								...
								-- Trigger the tool swing
								if AttackDelay < tick() then
									AttackDelay = tick() + (1 / CPS.GetRandomValue())
									pcall(function() tool:Activate() end)
								end

								if Lunge.Enabled and tool.GripUp ~= nil and tool.GripUp.X == 0 then break end
								if delta.Magnitude > AttackRange.Value then continue end

								-- Fire touch on enemy parts — must use a BasePart (Handle), not Tool
								if handle then
...
run(function()
	...
	local function Added(ent)
		...
		arrow.Visible = false
		arrow.Image = getcustomasset('KitVape/assets/new/arrowmodule.png')
	...
	local ESPAdded = {
		Drawing2D = function(ent)
			...
			if HealthBar.Enabled then
				...
				EntityESP.HealthLine.Color = Color3.fromHSV(math.clamp(ent.Health / (ent.MaxHealth > 0 and ent.MaxHealth or 100), 0, 1) / 2.5, 0.89, 0.75)
	...
	local ESPLoop = {
		Drawing2D = function()
			for _, ent in entitylib.List do
				if not Reference[ent] and ESPAdded.Drawing2D then
					ESPAdded.Drawing2D(ent)
				end
			end
			for ent, EntityESP in Reference do
				if not ent.RootPart or not ent.RootPart.Parent or not ent.Humanoid or not ent.Character or not ent.Character.Parent then
				...
				local lookCF = CFrame.new(ent.RootPart.Position, ent.RootPart.Position + gameCamera.CFrame.LookVector)
				local topPos = gameCamera:WorldToViewportPoint((lookCF * CFrame.new(2, ent.HipHeight or 3, 0)).p)
				local bottomPos = gameCamera:WorldToViewportPoint((lookCF * CFrame.new(-2, -(ent.HipHeight or 3) - 1, 0)).p)
				...
				if EntityESP.HealthLine then
					local maxH = ent.MaxHealth > 0 and ent.MaxHealth or 100
					local healthRatio = math.clamp(ent.Health / maxH, 0, 1)
					local healthposy = sizey * healthRatio
					EntityESP.HealthLine.Visible = ent.Health > 0
					EntityESP.HealthLine.From = Vector2.new(posx - 6, posy + sizey) // 1
					EntityESP.HealthLine.To = Vector2.new(posx - 6, posy + sizey - healthposy) // 1
					EntityESP.HealthBorder.From = Vector2.new(posx - 6, posy) // 1
					EntityESP.HealthBorder.To = Vector2.new(posx - 6, posy + sizey) // 1
				end
	
				if EntityESP.Text then
					EntityESP.Text.Position = Vector2.new(posx + (sizex / 2), posy - 20) // 1
		...
		Drawing3D = function()
			for ent, EntityESP in Reference do
				...
				local rootCF = ent.RootPart.CFrame
				local hh = ent.HipHeight or 3
				local point1 = ESPWorldToViewport((rootCF * CFrame.new(1.5, hh, 1.5)).p)
				local point2 = ESPWorldToViewport((rootCF * CFrame.new(1.5, -hh, 1.5)).p)
				local point3 = ESPWorldToViewport((rootCF * CFrame.new(-1.5, hh, 1.5)).p)
				local point4 = ESPWorldToViewport((rootCF * CFrame.new(-1.5, -hh, 1.5)).p)
				local point5 = ESPWorldToViewport((rootCF * CFrame.new(1.5, hh, -1.5)).p)
				local point6 = ESPWorldToViewport((rootCF * CFrame.new(1.5, -hh, -1.5)).p)
				local point7 = ESPWorldToViewport((rootCF * CFrame.new(-1.5, hh, -1.5)).p)
				local point8 = ESPWorldToViewport((rootCF * CFrame.new(-1.5, -hh, -1.5)).p)
		...
		DrawingSkeleton = function()
			for ent, EntityESP in Reference do
				...
				pcall(function()
					...
					local head = ESPWorldToViewport((ent.Head and ent.Head.CFrame or ent.RootPart.CFrame).p)
					local headfront = ESPWorldToViewport(((ent.Head and ent.Head.CFrame or ent.RootPart.CFrame) * CFrame.new(0, 0, -0.5)).p)

					local torsoPart = char:FindFirstChild(rigcheck and 'Torso' or 'UpperTorso') or ent.RootPart
					...
					local function getLimbCF(r6Name, r15Names)
						if rigcheck then
							local p = char:FindFirstChild(r6Name)
							return p and p.CFrame
						end
						for _, n in r15Names do
					...
					local leftArmCF = getLimbCF('Left Arm', {'LeftHand', 'LeftLowerArm', 'LeftUpperArm'})
					local rightArmCF = getLimbCF('Right Arm', {'RightHand', 'RightLowerArm', 'RightUpperArm'})
					local leftLegCF = getLimbCF('Left Leg', {'LeftFoot', 'LeftLowerLeg', 'LeftUpperLeg'})
					local rightLegCF = getLimbCF('Right Leg', {'RightFoot', 'RightLowerLeg', 'RightUpperLeg'})
					...
					if leftArmCF then
						...
						EntityESP.LeftArm.To = ESPWorldToViewport((leftArmCF * offset).p)
					end
					if rightArmCF then
						EntityESP.RightArm.From = toprighttorso
						EntityESP.RightArm.To = ESPWorldToViewport((rightArmCF * offset).p)
					end
					if leftLegCF then
						EntityESP.LeftLeg.From = bottomlefttorso
						EntityESP.LeftLeg.To = ESPWorldToViewport((leftLegCF * offset).p)
					end
					if rightLegCF then
						EntityESP.RightLeg.From = bottomrighttorso
						EntityESP.RightLeg.To = ESPWorldToViewport((rightLegCF * offset).p)
					end
				end)
			end
		end
...
run(function()
	...
	Radar = vape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('KitVape/assets/new/radaricon.png'),
...
run(function()
	...
	local function ChangeFFlag(suc)
		...
		if not success or typeof(json) ~= 'table' then
			notif('KitV4', 'Invalid json format for fflag', 12, 'warning')
			return
		end
		...
		notif('KitV4', 'FFlags applied, Go in a new game to take effect', 12, 'info')
	end
	...
	FFlag = vape.Legit:CreateModule({
		...
		Function = function(call)
			if call then
				...
				notif('KitV4', 'Inorder to disable fflags you have applied, You need to restart roblox', 20, 'info')
			end
		end
	})
