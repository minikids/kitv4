				...
				AnticheatBypass:Clean(runService.Heartbeat:Connect(function(dt)
					if entitylib.isAlive then
						...
						if diff.Magnitude > 6 and (os.clock() - tpTimer) > 0.75 then
							bypassRoot.CFrame = bypassRoot.CFrame + (clampVec(diff, entitylib.character.Humanoid.WalkSpeed))
							tpTimer = os.clock()
						end
						...
						if not flyCheck then
							...
							bypassRoot.CFrame = bypassRoot.CFrame - (Vector3.new(0, ((os.clock() - fallTimer) % 1) * 10, 0))
						else
							fallTimer = 0
						end
