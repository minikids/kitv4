...
local loadstring = function(...)
	...
	if err then
		warn('[KitV4 BedWars Load Error]: '..tostring(err))
		if vape then
			vape:CreateNotification('KitV4', 'Failed to load : '..tostring(err), 30, 'alert')
		end
	end
	return res
end
...
if isfile('KitVape/games/'..vape.Place..'.lua') then
	loadstring(downloadFile('KitVape/games/'..vape.Place..'.lua'), 'bedwars')()
else
