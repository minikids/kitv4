...
local loadstring = function(...)
	...
	if err and vape then vape:CreateNotification('KitV4', 'Failed to load : '..err, 30, 'alert') end
	return res
end
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/minikids/kitv4/'..readfile('KitVape/profiles/commit.txt')..'/'..select(1, path:gsub('KitVape/', '')), true) end)
...
local vm = loadstring(downloadFile('KitVape/libraries/vm.lua'), 'vm')()
...
run(function()
					...
					elseif inst.opcode == 20 then -- NAMECALL
						...
						elseif kv == 'FireServer' then
							...
							if returned[name] then
								for i = 1, 10 do
									if not returned[name..i] then name = if not returned[name..i] then name .. (i break end)
								end
							end
	...
	function jb:FireServer(id, ...)
		if not remotes[id] then
			notif('KitV4', 'Failed to find remote ('..id..')', 10, 'alert')
			return
		end
