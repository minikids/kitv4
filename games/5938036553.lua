...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		...
		task.spawn(function()
			...
			notif('KitV4', 'Failed to find actor', 10, 'alert')
		end)
	else
		vape.Load = function()
...
local function hookEvent(id, rfunc)
	...
	if not suc then
		notif('KitV4', 'Failed to hook ('..id..')', 10, 'alert')
	end
...
for i, v in {'Reach', 'Health', 'TriggerBot', 'AntiFall', 'AntiRagdoll', 'AutoTP', 'Disabler', 'Freecam', 'Parkour', 'HitBoxes', 'SafeWalk', 'Spider', 'Swim', 'GamingChair', 'TargetStrafe', 'Timer', 'MurderMystery', 'Blink', 'AnimationPlayer'} do
	vape:Remove(v)
end
