...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('KitVape/assets/new/blur.png')
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		local targetTeam = ent.Player and ent.Player:GetAttribute('Team')
		if myTeam == nil or targetTeam == nil or myTeam == '' or targetTeam == '' then
			return true
		end
...
run(function()
	...
	if not GlobalMiddleware or typeof(GlobalMiddleware) ~= "table" then
		notif('KitV4', 'Failed to load ratelimits, report this to a developer.', 30, 'alert')
	end
	...
	function RemoteHandler.Get(self, RemoteID: string)
		...
		Remote.LastRateLimitReset = os.clock()
		Remote.RequestTimestamps = {}
		...
		if not Success or not Remote.Remote then
			notif('KitV4', `Tried to Get remote {Remote.ID}, remote is invalid`, 15, 'alert')
			Remote.Remote = nil
		end
	...
	function RemoteHandler:Fire(Method: string?, ...)
		...
		if not self.Success or not Remote then
			if tick() - lastNotify > 0.5 then
				lastNotify = tick()
				notif('KitV4', `Tried to Fire remote {Remote.ID}, remote is invalid`, 10, 'alert')
			end
			return {
		...
		-- Rolling 60s window: drop timestamps older than 60s
		local now = os.clock()
		if not self.RequestTimestamps then self.RequestTimestamps = {} end
		while #self.RequestTimestamps > 0 and (now - self.RequestTimestamps[1]) >= 60 do
			table.remove(self.RequestTimestamps, 1)
		end

		if #self.RequestTimestamps >= self:GetRateLimit() then
			-- silently drop, no notification spam
	...
	function RemoteHandler:GetCurrentRequests()
		return #(self.RequestTimestamps or {})
	end

	function RemoteHandler:IncrementRequests()
		if not self.RequestTimestamps then self.RequestTimestamps = {} end
		table.insert(self.RequestTimestamps, os.clock())
		self.RequestsInLastMinute = #self.RequestTimestamps
	end
	...
	bedwars = setmetatable({
		...
		EmoteDisplayMeta = (function()
			local suc, res = pcall(function() return require(replicatedStorage.TS.locker.emote['emote-display-meta']).EmoteDisplayMeta end)
	...
	task.spawn(function()
		pcall(function()
			...
			-- Auto-enable AntiFlag on BedWars match start. Every
			-- BedWars cheat module below (Killaura, HitBoxes,
			-- FastBreak, InstantBreak, Fly, Speed) writes something
			-- the server flags on. AntiFlag's controller hooks are the
			-- floor; turning it on by default in the match ensures
			-- the user doesn't forget. Free-tier users (no premium)
			-- will silently no-op via the pcall below since
			-- vape.Modules.AntiFlag won't exist for them.
			pcall(function()
				local flag = vape.Modules and vape.Modules.AntiFlag
				if flag and not flag.Enabled then
					flag:Toggle()
				end
			end)
		...
		Function = function(callback)
			...
			if callback then
				...
				bedwars.BlockSelector.getMouseInfo = function(...)
					...
					if Select == 0 then
						Args.range = (BlockReach and BlockReach.Enabled and BlockRange and BlockRange.Value) or 24
					elseif Select == 1 then
						Args.range = (BreakReach and BreakReach.Enabled and BreakRange and BreakRange.Value) or 18
					end
			...
			if callback then
				...
				Fly:Clean(runService.PreSimulation:Connect(function(dt)
					if entitylib.isAlive and not InfiniteFly.Enabled and isnetworkowner(entitylib.character.RootPart) then
						...
						local velo = getSpeed()
						-- velocity is the per-tick target. With WallCheck we
						-- raycast the same vector against walls; if it hits
						-- we damp horizontal motion to zero instead of
						-- teleporting to the impact point.
						local horizontalVelo = moveDirection * math.max(Value.Value - velo, 0)
						rayCheck.FilterDescendantsInstances = {lplr.Character, gameCamera, AntiFallPart}
						rayCheck.CollisionGroup = root.CollisionGroup

						if WallCheck.Enabled and horizontalVelo.Magnitude > 0 then
							local ray = workspace:Raycast(root.Position, horizontalVelo * dt, rayCheck)
							if ray then
								horizontalVelo = Vector3.zero
							end
						end
							...
							if tpToggle then
								...
								if airleft > 2 then
									if not oldy then
										...
										if ray and TP.Enabled then
											...
											tpTick = tick() + 0.11
											-- Single-shot Y teleport to snap back to ground.
											-- Wrapped in pcall so a missing/welded root
											-- doesn't break the fly loop. The server
											-- will rubber-band once on this transition,
											-- then per-frame velocity below looks legit.
											pcall(function()
								...
								if oldy then
									if tpTick < tick() then
										...
										-- One-time Y teleport to recover original
										-- height after TP-Down. Same caveat as above.
										pcall(function()
						...
						-- Velocity-only movement. We translate the per-frame
						-- destination into a velocity vector for this tick so
						-- the server sees a legitimate motion step rather than
						-- a hard teleport. Combined with AntiFlag's
						-- LagController + FlyController hooks this should stop
						-- the per-tick rubber-banding on Fly.
						root.AssemblyLinearVelocity = (moveDirection * velo) + horizontalVelo + Vector3.new(0, mass, 0)
					end
				end))
					...
					if sword then
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								local itemType = (sword and sword.itemType) or (sword and sword.tool and sword.tool.Name)
								local itemMeta = itemType and bedwars.ItemMeta[itemType]
								local Meta = itemMeta and itemMeta.sword
								local attackSpeed = Meta and Meta.attackSpeed or 0.2
								local BaseCooldown = math.max((bedwars.SwordController.lastAttack + attackSpeed) - workspace:GetServerTimeNow(), 0) + (store.ping.incoming - 0.0165)
...
run(function()
	...
	local AutoDisable
	local Cooldown
	local lastJumpTick = nil

	local function jump()
		local cd = Cooldown and Cooldown.Value or 0.3
		if lastJumpTick and (tick() - lastJumpTick) < cd then
			return
		end
		lastJumpTick = tick()
		...
		HighJump = vape.Categories.Blatant:CreateModule({
			...
			Function = function(callback)
				if callback then
					...
					pcall(function()
						local fly = vape.Modules and vape.Modules.Fly
		...
		Cooldown = HighJump:CreateSlider({
			Name = 'Cooldown',
			Min = 0.3,
			Max = 5,
			Default = 0.3,
...
run(function()
		...
		Function = function(callback)
			if callback then
				...
				bedwars.ProjectileController.calculateImportantLaunchValues = function(...)
					...
					if plr then
						if math.random(0, 100) > Chance.Value then
							return old(...)
						end
	...
	Chance = ProjectileAimbot:CreateSlider({
		Name = 'Chance',
		Min = 0,
		Max = 100,
		Default = 100,
		Suffix = '%',
		Tooltip = 'Probability that Projectile Aimbot is applied per shot. 100% = always, 0% = never.'
	})
end)
			...
			if callback then
				...
				Speed:Clean(runService.PreSimulation:Connect(function(dt)
					...
					if entitylib.isAlive and not Fly.Enabled and not InfiniteFly.Enabled and not LongJump.Enabled and isnetworkowner(entitylib.character.RootPart) then
						...
						-- Same velocity-only rewrite as the Fly module:
						-- convert the per-tick delta into a horizontal
						-- velocity component so the server sees a single
						-- legit motion step, not a teleport.
						local horizontalVelo = moveDirection * math.max(Value.Value - velo, 0)
						...
						root.AssemblyLinearVelocity = (moveDirection * velo) + horizontalVelo + Vector3.new(0, root.AssemblyLinearVelocity.Y, 0)
			...
			if callback then
				BulletTracers:Clean(workspace.ChildAdded:Connect(function(projectile)
					task.delay(0, function()
						...
						if prediction and type(prediction.SpawnArcTracer) == "function" then
...
run(function()
	...
	local function honor()
	    ...
	    local list, team = {}, lplr:GetAttribute('Team')
	    for _, v in entitylib.List do
	    ...
	    table.sort(list, function(a, b)
	        local aTeam = a.Player and a.Player:GetAttribute('Team')
	        local bTeam = b.Player and b.Player:GetAttribute('Team')
	        return aTeam == team and bTeam ~= team
	    end)
	    for _, v in list do
	        if #Honored > 1 then break end
	        if v.Player and not table.find(Honored, v.Player) then
...
run(function()
	...
	for i, v in bedwars.EmoteMeta do
	    if (not bedwars.EmoteType or i ~= bedwars.EmoteType.NONE) and v and v.name and not old[v.name] then
	        old[v.name] = i
	        table.insert(list, v.name)
	    end
	end
	...
	local function cancelEmote()
	    if entitylib.isAlive then
	        ...
	        -- Also remove any BillboardGui left over from emotes that
	        -- use a sprite overlay instead of a humanoid animation
	        -- (e.g. the Nightmare star). Walk back up to the head and
	        -- destroy any BillboardGui named after the emote we set.
	...
	SetEmote = vape.Categories.Utility:CreateModule({
	    ...
	    Function = function(callback)
	        if callback then
	            SetEmote:Toggle()
	            -- Wrap the rest in pcall so an API drift in the
	            -- BedWars emote framework (e.g. methods being renamed
	            -- or removed in a game update) doesn't kill the module.
	            -- Previously, an error from bedwars.EmoteController:
	            -- playEmoteBeginSounds (which no longer exists on this
	            -- controller) aborted the whole Function callback before
	            -- the BillboardGui / animation code could run, leaving
	            -- the user stuck with a dangling star and no way to
	            -- cancel except to re-toggle the module.
	            local suc, err = pcall(function()
	                ...
	                local display = meta.emoteDisplayType and bedwars.EmoteDisplayMeta and bedwars.EmoteDisplayMeta[meta.emoteDisplayType]

	                -- Helper to play any sound found in metadata
	                local function playEmoteSound(soundData)
	                    if not soundData then return end
	                    if type(soundData) == 'table' then
	                        for _, s in soundData do
	                            playEmoteSound(s)
	                        end
	                    elseif type(soundData) == 'string' or type(soundData) == 'number' then
	                        bedwars.SoundManager:playSound(soundData)
	                    end
	                end

	                -- Play sounds from meta and display properties
	                playEmoteSound(meta.beginSounds)
	                playEmoteSound(meta.sound)
	                playEmoteSound(meta.sounds)
	                playEmoteSound(meta.soundId)
	                playEmoteSound(meta.song)
	                playEmoteSound(meta.audio)

	                if display then
	                    playEmoteSound(display.beginSounds)
	                    playEmoteSound(display.sound)
	                    playEmoteSound(display.sounds)
	                    playEmoteSound(display.soundId)
	                    playEmoteSound(display.song)
	                    playEmoteSound(display.audio)
	                end

	                local animation = meta.animation or (display and display.animation)
	                ...
	                local imageAsset = meta.image or (display and display.image)
	                    or meta.icon or (display and display.icon)
	                    or meta.imageId or (display and display.imageId)
	                    or meta.texture or (display and display.texture)
	                    or meta.assetId or (display and display.assetId)
	                ...
	                if imageAsset and imageAsset ~= '' then
	                    ...
	                    gui.Adornee = lplr.Character:FindFirstChild('Head') or lplr.Character.PrimaryPart
	                    ...
	                    gui.Parent = lplr.Character:FindFirstChild('Head') or lplr.Character.PrimaryPart
	            ...
	            if not suc and shared.VapeDeveloper then
	                warn('SetEmote error:', err)
	            end
	        end
	    end,
...
run(function()
	...
	SetSettings = vape.Categories.Utility:CreateModule({
	    ...
	    Tooltip = 'Adds bedwars settings options to KitV4 (also carries the settings with your cv config).'
	})
	...
	for i, v in old do
	    ...
	    elseif shared.VapeDeveloper then
	        notif('KitV4', 'Unknown bedwars setting detected ('.. i.. ')', 20, 'alert')
	    end
	end
end)
...
run(function()
	...
	local function staffFunction(plr, checktype)
		...
		-- Closet mode: silently self-destruct the GUI immediately when staff is detected
...
run(function()
	...
	vape.Components.HotbarList = function(optionsettings, children, api)
		...
		textbuttonicon.Image = getcustomasset('KitVape/assets/new/add.png')
	            ...
	            local mounted = bedwars.Roact.mount(create('BillboardGui', {
	                ...
	                create('Frame', {
	                    ...
	                    create('ImageLabel', {
	                        ...
	                        BackgroundTransparency = 1,
	                        Image = getcustomasset('KitVape/assets/new/blur.png'),
	...
	local function attemptBreak(tab, localPosition)
		...
		for _, v in tab do
			if (v.Position - localPosition).Magnitude < Range.Value and bedwars.BlockController:isBlockBreakable({blockPosition = v.Position / 3}, lplr) then
				...
				hit += 1
				-- Normal path: use the shared helper which path-finds,
				-- switches tools, updates the visual highlight dots and
				-- the healthbar, etc.
				...
				-- Pick the per-hit delay. FastBreak (Blatant
				-- category) is the shared per-hit cooldown for the
				-- game's normal hit handler; the Breaker bypasses
				-- that handler entirely (it calls bedwars.breakBlock
				-- directly), so without this it would ignore whatever
				-- Time the user set on FastBreak. When FastBreak is on,
				-- honour its Time so a single Time value drives both
				-- the regular swing and the Breaker.
				local _fb = vape.Modules.FastBreak
				local _interhit = (_fb and _fb.Enabled and _fb.Options and _fb.Options.Time and _fb.Options.Time.Value)
						or BreakSpeed.Value
				task.wait(_interhit)
