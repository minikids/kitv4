...
local drawingactor = loadstring(downloadFile('KitVape/libraries/drawing.lua'), 'drawing')(...)
local redline = {Teams = {}}
...
local function warningRoutine(hash)
	local path = 'KitVape/profiles/agreementhash.txt'
...
if not select(1, ...) then
	if run_on_actor then
		...
		task.spawn(function()
			...
			local executionString = "loadfile('KitVape/main.lua')("..drawingactor..")"
		...
		vape.Load = function()
			notif('KitV4', 'Missing actor functions.', 10, 'alert')
		end
	end
...
for _, v in {'Reach', 'TriggerBot', 'AntiFall', 'Desync', 'HitBoxes', 'AutoTP', 'Jesus', 'MouseTP', 'Spider', 'SpinBot', 'Swim', 'TargetStrafe', 'AntiRagdoll', 'Disabler', 'StateSpoofer', 'Parkour', 'SafeWalk', 'MurderMystery'} do
	vape:Remove(v)
end
