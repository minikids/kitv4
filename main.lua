local license = ... or {}
shared.closetMode = license.Closet or false
...
local function finishLoading()
	...
	if not shared.vapereload then
		...
		if not shared.vapereload then
			-- Clear any catname/catrole left over from a previous version of
			-- the script so it doesn't show up in this notification or any
			-- other UI element. The previous notification included the
			-- user's Discord identity (set by a now-stripped auth backend)
			-- which leaked CatVape-only usernames; the replacement is just
			-- the keybind hint.
			getgenv().catname = nil
			getgenv().catrole = nil
			if not shared.closetMode then
				vape:CreateNotification('KitV4', (vape.VapeButton and 'Press the button in the top right' or 'Press '..table.concat(vape.Keybind, ' + '):upper())..' to open GUI', 5)
			end
			task.delay(0.05 + cloneref(game:GetService('RunService')).PostSimulation:Wait(), function()
				if shared.updated and not shared.closetMode then
...
if hookmetamethod and checkcaller and getnamecallmethod then
	local old
	local ok, hooked = pcall(hookmetamethod, game, '__namecall', function(self, Remote, ...)
...
if not shared.VapeIndependent then
	...
	-- Skip premium.lua in the BedWars lobby (PlaceId 6872265039).
	-- The lobby doesn't load the bedwars game framework, so the
	-- premium module block would warn "bedwars is not defined" and
	-- define zero modules. The lobby-specific file already provides
	-- the legit lobby surface (Sprint, AutoGamble, Render ESPs).
