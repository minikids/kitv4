...
entitylib.EntityMouse = function(entitysettings)
	if entitylib.isAlive then
		...
		for _, entity in entitylib.List do
			if (entitysettings.Players or not entity.Player) and (entitysettings.NPCs or not entity.NPC) and entity.Targetable then
				...
				if vis then
					...
					if mag <= entitysettings.Range and entitylib.isVulnerable(entity) then
		...
		for _, v in sortingTable do
			if not (entitysettings.Wallcheck and entitylib.Wallcheck(entitysettings.Origin, v.Entity[entitysettings.Part].Position, entitysettings.Wallcheck)) then
...
entitylib.EntityPosition = function(entitysettings)
	if entitylib.isAlive then
		...
		for _, v in sortingTable do
			if not (entitysettings.Wallcheck and entitylib.Wallcheck(localPosition, v.Entity[entitysettings.Part].Position, entitysettings.Wallcheck)) then
				table.clear(entitysettings)
				table.clear(sortingTable)
				return v.Entity
			end
